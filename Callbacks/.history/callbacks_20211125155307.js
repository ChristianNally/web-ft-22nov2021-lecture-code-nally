...
// doSomethingWithThisString('Monkey Fuzz!!'); // notice the syntax, and how the variable can now be executed since it is function valued
...
// we would use function-valued parameters when we need to abstract out some action, but still be able to write
// code about how that abstract action will be used.
...
// const lightItUp = function(param){
//   console.log(`LET THERE BE LIGHT: ${param}`);
// };


// performYourActionThreeTimes( lightItUp );
...
const complexAction = function(paramOne,paramTwo){
  const result = paramTwo * paramTwo + 50;
  console.log(`${paramOne}  :::  ${result} `);
};

performYourComplexActionThreeTimes( function(paramOne,paramTwo){
  const result = paramTwo * paramTwo + 50;
  console.log(`${paramOne}  :::  ${result} `);
} );


// const blah = "any other string";
// print('monkey fuzz');
// print(blah);
...
performYourComplexActionThreeTimes( function(paramOne,paramTwo){
  const result = paramTwo * paramTwo * paramTwo + 50 * paramTwo;
  console.log(` MONKEY FUZZ!  :::  ${result} `);
} );
...
// param => console.log('MonkeyFuzz!:', param)
